import type { DeleteTicketComment, PostTicketComment, PutTicketComment, GetTicketComment } from "$lib/models/tickets/ticket-comments.type";
import type { Pagination } from "$lib/models/common/common.type";
import { get, writable } from "svelte/store";
import { deleteTicketComment, getTicketComments, postTicketComment, putTicketComment } from "$lib/services/tickets/ticket-comment.service";
...
export const ticketCommentsActions = {
  ...
  putTicketComment: async (ticket_id: string, body: PutTicketComment) => {
    ...
    try {
      ticketCommentsStore.update(prev => prev.map(c => c.id === body.id ? {...c, ...body, created_by: c.created_by} : c));
