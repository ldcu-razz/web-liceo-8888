...
import {
	archiveTicketCategory,
	createTicketCategory,
	getTicketCategories,
	updateTicketCategory
} from '$lib/services/tickets/ticket-categories.service';
...
export const ticketCategoriesNonArchivedStore = derived(ticketCategoriesAll, (all) =>
	all.filter((c) => c.status !== BaseStatusEnumSchema.enum.archived)
);
...
export const ticketCategoriesActions = {
	...
	getTicketCategories: async (pagination: Pagination, q?: string, silentLoading?: boolean) => {
		try {
			...
			ticketCategoriesPagination.update((prev) => ({
				...prev,
	...
	updateTicketCategory: async (id: string, payload: PutTicketCategories) => {
		...
		try {
			...
			ticketCategoriesStore.update((prev) =>
				prev.map((c) => (c.id === id ? { ...c, ...payload } : c))
			);
			ticketCategoriesAll.update((prev) =>
				prev.map((c) => (c.id === id ? { ...c, ...payload } : c))
			);
		} catch (error) {
			console.error(error);
			if (currentUpdatedTicketCategory) {
				ticketCategoriesStore.update((prev) =>
					prev.map((c) => (c.id === id ? { ...currentUpdatedTicketCategory } : c))
				);
			}
			ticketCategoriesError.set((error as Error).message);
		}
	...
	archiveTicketCategory: async (id: string) => {
		...
		try {
			...
			ticketCategoriesStore.update((prev) =>
				prev.map((c) =>
					c.id === id ? { ...c, status: BaseStatusEnumSchema.enum.archived, updatedAt } : c
				)
			);
			ticketCategoriesAll.update((prev) =>
				prev.map((c) =>
			...
			if (currentArchivedTicketCategory) {
				ticketCategoriesStore.update((prev) =>
					prev.map((c) => (c.id === id ? { ...currentArchivedTicketCategory } : c))
				);
			}
