...
<div class="mt-4 rounded-lg border border-border">
	...
	>
      ...
      <div class="flex items-center gap-2">
        ...
        <Button variant="outline" size="sm" disabled={remainingTicketsCreation === userProperties?.remaining_tickets_creation || !remainingTicketsCreation} onclick={() => handleUpdateUserProperties({ remaining_tickets_creation: remainingTicketsCreation ?? undefined })}>
          Save
        </Button>
      </div>
		</div>
	...
	>
		<div class="flex items-center justify-between gap-2 flex-1">
			...
			<Switch id="bypass-ticket-creation-limit" bind:checked={bypassTicketCreationLimit} onCheckedChange={() => handleUpdateUserProperties({ bypass_ticket_creation_limit: bypassTicketCreationLimit })} />
		</div>
	</div>
</div>
