<script lang="ts">
	import { AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle } from '$lib/components/ui/alert-dialog';
	...
	import { systemSettingsActions, systemSettingsLoadingStore, systemSettingsStore } from '$lib/store/system-settings.store';
  ...
  function handleUpdateSystemSettings(payload: PutSystemSettings) {
    ...
    if (payload.number_of_tickets_creation_limit && $systemSettingsStore?.number_of_tickets_creation_limit === ticketCreationLimit) {
      return;
    }

    if (payload.number_of_users_creation_limit && $systemSettingsStore?.number_of_users_creation_limit === numberOfUsersCreationLimit) {
      return;
    }

    if (payload.is_maintenance_mode && $systemSettingsStore?.is_maintenance_mode === maintenanceMode) {
      return;
    }
...
{:else}
  <div class="h-full p-8 bg-white">
    ...
    <div class="grid grid-cols-12 auto-rows-fr gap-4 rounded-md bg-white">
      <div class="col-span-6 h-full">
        <div class="border border-border rounded-md p-4 flex flex-col gap-2 h-full">
          ...
          <Input type="number" bind:value={ticketCreationLimit} onblur={() => handleUpdateSystemSettings({ number_of_tickets_creation_limit: ticketCreationLimit })} />
        </div>
      </div>
      ...
      <div class="col-span-6 h-full">
        <div class="border border-border rounded-md p-4 flex flex-col gap-2 h-full">
          ...
          <Input type="number" bind:value={numberOfUsersCreationLimit} onblur={() => handleUpdateSystemSettings({ number_of_users_creation_limit: numberOfUsersCreationLimit })} />
        </div>
      </div>
      ...
      <div class="col-span-6 h-full">
        <div class="border border-border rounded-md p-4 flex flex-col gap-2 h-full">
          ...
          <Switch id="maintenance-mode" bind:checked={maintenanceMode} onCheckedChange={() => handleUpdateSystemSettings({ is_maintenance_mode: maintenanceMode })} />
        </div>
      </div>
      ...
      <div class="col-span-6 h-full">
        <div class="border border-border rounded-md p-4 flex flex-col gap-2 h-full">
          ...
          <div class="flex justify-start mt-3">
            <Button variant="destructive" size="sm" disabled={resetAllUsersTicketLimitLoading} onclick={() => handleShowResetAllUsersTicketLimitAlertDialog()}>Reset All Users Ticket Limit</Button>
          </div>
        </div>
      </div>
    </div>
  ...
  <AlertDialogContent>
    ...
    <AlertDialogDescription>
      {#if resetAllUsersTicketLimitLoading}
        ...
        <div>Please wait for the action to complete. Don't close the page or refresh the page until the action is complete.</div>
      {:else}
        Resetting the ticket limit for all users will reset the ticket limit for all users to the system settings ticket limit. Please note that this action is irreversible.
      {/if}
    </AlertDialogDescription>
    <AlertDialogFooter>
      <Button variant="destructive" disabled={resetAllUsersTicketLimitLoading} onclick={() => handleResetAllUsersTicketLimit()}>
        {#if resetAllUsersTicketLimitLoading}
          <Loader class="size-4 animate-spin" />
        {/if}
        Yes, Reset
      </Button>
      <Button variant="outline" disabled={resetAllUsersTicketLimitLoading} onclick={() => (showResetAllUsersTicketLimitAlertDialog = false)}>Cancel</Button>
    </AlertDialogFooter>
  </AlertDialogContent>
</AlertDialog>