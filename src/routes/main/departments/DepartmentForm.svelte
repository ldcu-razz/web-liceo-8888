...
<script lang="ts">
	...
	let touched = $state<Record<keyof FormData, boolean>>(createInitialTouched(formData));
...
<form onsubmit={handleSubmit}>
	<FieldGroup>
		<Field>
			<FieldLabel for="department-name" class="gap-1"
				>Department name<span class="text-red-500">*</span></FieldLabel
			>
			<Input
				id="department-name"
				type="text"
		...
		<Field>
			<FieldLabel for="abbreviation" class="gap-1"
				>Abbreviation<span class="text-red-500">*</span></FieldLabel
			>
			<Input
				id="abbreviation"
				type="text"
					...
					{#each formData.keywords as keyword (keyword)}
						...
						>
							...
							<button
								...
								class="ml-1 rounded-full p-0.5 transition-colors hover:bg-gray-100"
	...
	<div class="mt-6 flex gap-2">
		...
		<Button
			...
			variant="secondary"
			class="flex-1"
			disabled={disabledPositiveButton || (isFormTouched && invalid)}>Save</Button
		>
	</div>
</form>
