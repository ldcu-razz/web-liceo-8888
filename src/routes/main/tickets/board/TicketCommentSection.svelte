<script lang="ts" module>
	...
	import type { PostTicketComment, DeleteTicketComment, PutTicketComment } from "$lib/models/tickets/ticket-comments.type";
	import type { MentionedUsers } from "$lib/models/users/users.type";
	import { meStore } from "$lib/store/me.store";
  import { ticketCommentsActions, ticketCommentsLoadingStore, ticketCommentsStore } from "$lib/store/ticket-comments.store";
...
<div class="flex flex-col h-full gap-2">
  {#if ticketCommentsLoading}
    <div class="flex flex-col justify-center gap-3 mt-auto">
  ...
  {:else}
    <div class="mt-auto">
      <RichTextEditor bind:value={commentValue} bind:mentionedUsers placeholder="Write a comment" onSubmit={handleSubmitComment} />
    </div>
    {#each ticketComments as comment (comment.id)}
      <TicketCommentItem 
        ...
        onSaveEdit={(value: string, mentionedUsers: MentionedUsers[]) => handleSaveEdit(comment.id, mentionedUsers, value)}
      />
    {/each}
  {/if}
</div>
...
<AlertDialog.Root open={deleteCommentId !== null} onOpenChange={() => deleteCommentId = null}>
  <AlertDialog.Content>
    ...
    <AlertDialog.Footer>
      ...
      <AlertDialog.Action onclick={confirmDelete} class="bg-red-600 hover:bg-red-700">Delete</AlertDialog.Action>
    </AlertDialog.Footer>
  </AlertDialog.Content>
</AlertDialog.Root>