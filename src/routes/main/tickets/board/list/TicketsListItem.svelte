...
<script lang="ts">
	...
	let assignedDepartmentInitial = $derived(
		...
		<ItemContent>
			<div class="flex justify-between gap-4">
				<div class="flex items-center gap-2">
					...
					<span class="line-clamp-1 text-sm font-medium text-ellipsis">{ticket.code}</span>
					<span class="line-clamp-1 text-sm font-medium text-ellipsis">{ticket.title}</span>
				</div>
						...
						<TooltipTrigger>
							<Avatar class="size-6">
								...
								<AvatarFallback class="text-xs"
									>{assignedUser ? assignedUserInitial : assignedDepartmentInitial}</AvatarFallback
								>
							</Avatar>
						</TooltipTrigger>
