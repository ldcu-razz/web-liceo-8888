<script lang="ts" module>
  ...
  import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "$lib/components/ui/table";
	...
	import { ticketCategoriesActions, ticketCategoriesPagination, ticketCategoriesTotalCount } from "$lib/store/ticket-categories.store";
...
<script lang="ts">
  ...
  let { columns, data, loading = $bindable(false) }: DataTableProps<TicketCategories, unknown> = $props();

  let pagination = $derived($ticketCategoriesPagination);

  let totalCount = $derived($ticketCategoriesTotalCount);
  ...
  async function handlePageChange(page: number) {
    await ticketCategoriesActions.getTicketCategories({ page: page, size: pagination.size });
