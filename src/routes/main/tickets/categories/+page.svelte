<script lang="ts">
	...
	import TicketCategoryForm, {
		defaultFormData,
		type FormData as TicketCategoryFormData,
		type Mode as TicketCategoryFormMode
	} from './TicketsCategoryForm.svelte';
	import {
		ticketCategoriesActions,
		ticketCategoriesLoading,
		ticketCategoriesStore
	} from '$lib/store/ticket-categories.store';
	...
	let activeTicketCategory: TicketCategories | null = $derived(
		categories.find((c) => c.id === activeTicketCategoryId) ?? null
	);
	...
	const columns = $derived(
		createTicketsCategoriesColumns(handleViewTicketCategory, handleTicketCategoryArchive)
	);
					...
					<SheetHeader>
						...
						<SheetDescription
							>{formMode === 'create'
								...
								: 'Edit the ticket category information.'}</SheetDescription
						>
					</SheetHeader>
					<TicketCategoryForm
						bind:formData
						onSubmit={handleSubmitTicketCategoryForm}
						onCancel={handleCancelTicketCategoryForm}
					/>
				</SheetContent>
			</SheetTrigger>
		</Sheet>
...
<AlertDialog bind:open={showDeleteTicketCategoryAlertDialog}>
	<AlertDialogContent>
		<AlertDialogHeader>
			<AlertDialogTitle
				>Are you sure to archive the ticket category {activeTicketCategory?.name}?</AlertDialogTitle
			>
			<AlertDialogDescription
				>Archiving this ticket category will remove it from the system.</AlertDialogDescription
			>
		</AlertDialogHeader>
		<AlertDialogFooter>
			<Button variant="destructive" onclick={handleArchiveTicketCategory}>Archive</Button>
			<Button variant="outline" onclick={() => (showDeleteTicketCategoryAlertDialog = false)}
				>Cancel</Button
			>
		</AlertDialogFooter>
	</AlertDialogContent>
